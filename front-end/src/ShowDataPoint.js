...
class ShowDataPoint extends React.Component {
    constructor(props) {
      ...
      componentDidMount() {
          ...
          fetch('//localhost:8765/evcharge/api/chargingPoints/', requestOptions1)
              .then((response) => {
                  return response.json();
              })
              .then((data) => {
                  data.forEach(function (item) {
                      if (item.id == localStorage.getItem("PointDataId")) {
                          localStorage.setItem("Operatortitle",item.operator.Title)
                      }
                  })
              })
              .catch(error => {
